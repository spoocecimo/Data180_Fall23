...
```{r}
recordsyear <- records %>% mutate(year=(record_duration>365))
summary<- recordsyear %>% summarize(s=sum(year))
head(summary)
```
# Question 3
...
```{r}
recordsmean <- records %>% group_by(track) %>% summarize(avg_time=mean(time))
recordsmean
```
# Question 5
...
```{r}
recordsmean <- records %>% group_by(year) %>% summarize(avg_time=mean(time))
recordsmean
```
# Question 7
...
```{r, fig.width=10}
recordsmean <- records %>% group_by(year) %>% summarize(avg_time=mean(time))
recordsmean %>% ggplot(aes(year,avg_time))+geom_bar(stat='identity')
```
...
```{r}
records %>% group_by(type) %>% ggplot(aes(type,time)) +geom_histogram(stat='identity')
```
...
records %>% group_by(type) %>% ggplot(aes(type,time_per))+geom_histogram(stat='identity')
```
# Question 10
...
```{r}
records%>% group_by(type) %>% filter(shortcut=='No') %>% ggplot(aes(type,time_per)) + geom_histogram(stat='identity')
```
...
```{r}
recordssystem <- records %>% group_by(system_played) %>% summarize(avg_time=mean(time))
recordssystem
```
