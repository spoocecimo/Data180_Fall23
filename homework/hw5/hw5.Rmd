...
```{r}
distance<-round(dist(Vans93[,-1]),digits=2)
Vans_Euclidean<-as.matrix(distance) 
colnames(Vans_Euclidean)<-Vans93[,1]
rownames(Vans_Euclidean)<-Vans93[,1]
Vans_Euclidean
#shortest distance is Caravan and Aerostar and longest distance is Lumina_APV and Eurovan
```
...
```{r}
distance<-round(dist(Vans93[,-1],method='manhattan'),digits=2)
Vans_Manhattan<-as.matrix(distance) 
...
My ranking shows the most used words, however the biggest problem is that 3 catorgories have 1 word while the second one has 6 words. This was still the best way to break it up. 
# Question 5
...
vertebrate_euclidean<-dist(Ver_Euclidean)
eudclidian<-hclust(vertebrate_euclidean,method='complete')
plot(eudclidian,lwd=3,main='Clustered vertebrates, Height over Vertebrates',sub="",xlab="vertebrates",cex.lab=1.25,hang=-1)
```
...
vertebrate_Manhattan<-dist(Ver_Manhattan)
Manhattan<-hclust(vertebrate_Manhattan,method='single')
plot(Manhattan,lwd=3,main='Clustered vertebrates, Height over Vertebrates',sub="",xlab="vertebrates",cex.lab=1.25,hang=-1)
```
